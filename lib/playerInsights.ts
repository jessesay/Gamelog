...
export function playerInsights(logs: any[], preferredGenres: string[] = [], preferredPlatforms: string[] = []) {
  const counts = { saved: 0, playing: 0, completed: 0, dropped: 0 };
  ...
  for (const log of logs) {
    ...
    if (["want_to_play", "wishlist"].includes(status ?? "")) { counts.saved += 1; if (game) backlog.push(game); }
  ...
  const recommendations = [...backlog].sort((a, b) => recommendationScore(b) - recommendationScore(a) || String(a.title).localeCompare(String(b.title))).slice(0, 3).map((game, index) => {
    ...
    const reasons = [sharedGenre ? `Fits your ${sharedGenre} streak` : null, sharedPlatform ? `Ready on ${sharedPlatform}` : null, index === 0 ? "Best current backlog fit" : index === 1 ? "Strong change-of-pace pick" : "Wild card from your saved shelf"].filter(Boolean).slice(0, 3) as string[];
    return { game, percentage, reasons };
  });
  const rawScore = 38 + Math.min(20, counts.saved * 2) + Math.min(14, counts.playing * 5) + Math.min(28, counts.completed * 3) + Math.min(6, counts.dropped);
