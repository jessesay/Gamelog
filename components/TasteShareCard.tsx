...
export default function TasteShareCard({ ownerName, username, genres, platforms, backlogCount, backlogScore, scoreLabel, recommendation, shareable = true }: { ownerName: string; username: string; genres: string[]; platforms: string[]; backlogCount: number; backlogScore: number; scoreLabel: string; recommendation?: { game: any; percentage: number; reasons: string[] }; shareable?: boolean }) {
  return <section className="taste-share-wrap-v315">
    <article className="taste-share-card-v315">
      ...
      <div className="taste-share-owner-v315"><p>@{username}</p><h2>{ownerName}&apos;s taste, logged.</h2><div className="taste-share-tags-v315">{genres.slice(0, 3).map((item) => <span key={item}>{item}</span>)}{platforms.slice(0, 2).map((item) => <span key={item}>{item}</span>)}</div></div>
