...
export default function GameSwipeDeck() {
  ...
  async function swipe(action: "liked" | "disliked" | "skipped" | "saved") {
    ...
    if (action === "saved") {
      setLastAction(`${swipedGame.title} saved to backlog`);
    } else if (action === "disliked") {
      setLastAction(`${swipedGame.title} passed`);
    } else {
      setLastAction(`${swipedGame.title} skipped`);
    }
      ...
      <main style={styles.page}>
        <div style={styles.shell}>
          <nav style={styles.nav}>
            <Link href="/backlog" style={styles.navLink}>
              My Backlog →
            </Link>
          </nav>
          <p style={styles.message}>{message}</p>
  ...
  return (
    <main style={styles.page}>
      <div style={styles.shell}>
        <nav style={styles.nav}>
          <span style={styles.logo}>GameLog</span>
          <Link href="/backlog" style={styles.navLink}>
            My Backlog →
          </Link>
        </nav>
        ...
        <div style={styles.topBar}>
          <div>
            <p style={styles.kicker}>Discovery Feed</p>
        ...
        <section style={styles.card}>
          <div
            style={{
              ...styles.cover,
              backgroundImage: currentGame.cover_url
                ? `linear-gradient(to bottom, rgba(0,0,0,0.05), rgba(0,0,0,0.78)), url(${currentGame.cover_url})`
            ...
            <div style={styles.cardContent}>
              ...
              <div style={styles.metaRow}>
                ...
                {currentGame.metacritic ? (
                  <span>Meta {currentGame.metacritic}</span>
                ) : null}
        ...
        <p style={styles.hint}>
          X = pass. Maybe = skip for now. Check = save to backlog.
        </p>

        {lastAction ? <p style={styles.toast}>{lastAction}</p> : null}
      </div>
    </main>
  );
}
